    ...
    return (
      <main className="mx-auto max-w-4xl px-4 py-8">
        ...
        <div className="mt-4 flex flex-wrap gap-3">
          ...
          {showRelogin ? (
            <Link
              href={`/admin/login?next=${encodeURIComponent(`/admin/jobs/${encodeURIComponent(String(id ?? ""))}`)}`}
  ...
  return (
    <main className="mx-auto max-w-5xl px-4 py-8">
      <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">{t("Редагування вакансії", "求人を編集", "Edit job")}</h1>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          >
            {published ? t("Зняти з сайту", "非公開にする", "Unpublish") : t("Опублікувати", "公開する", "Publish")}
          </button>
          ...
          {msgErr && (
            <div className="rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-sm text-red-800">
              ...
              {isUnauthorizedText(msgErr) ? (
                <div className="mt-2">
                  <Link className="font-semibold underline underline-offset-4" href={`/admin/login?next=${encodeURIComponent(`/admin/jobs/${encodeURIComponent(String(id ?? ""))}`)}`}>
                    {t("Перелогінитись", "再ログイン", "Re-login")}
                  </Link>
                </div>
              ) : null}
